...
requireLogin();
$id = intval($_GET['id'] ?? 0);
$owner_id = intval($_GET['owner_id'] ?? 0);   // nhận owner_id (nếu có)
...
<script>
  $('#vehicleForm').on('submit', function(e) {
    e.preventDefault();
    $.post('../vehicles/ajax/save_vehicle.php', $(this).serialize(), res => {
      if (res.success) {
        (bootstrap.Modal.getInstance('#ownerModal') ||
          bootstrap.Modal.getInstance('#vehicleModal')).hide();
        if (typeof loadOwners === 'function') loadOwners(); // refresh bảng chủ sở hữu
